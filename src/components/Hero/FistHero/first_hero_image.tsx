...
function FirstHeroImage({ src, title, index, showContent }: { src: string, title: string, index: number, showContent: boolean }) {
    ...
    return (
        <div className="relative" onMouseEnter={() => setIsHovered(index)} onMouseLeave={() => setIsHovered(null)}>
            <div className={`absolute w-full z-[2] pointer-events-none text-white ${isHovered === index ? "inset-y-[45%]" : "inset-y-1/2"} text-center text-5xl font-spins animate__animated animate__fadeInDownBig animate__delay-${index + 1}s ${showContent && 'duration-200'}`}>{title}</div>
